...
# Initialize curl counter and stage variables
counter = 0
stage = None
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        # Convert the image color from BGR to RGB for processing
        ...
        # Convert the image color back from RGB to BGR for displaying
        image.flags.writeable = True
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

        # Proceed if landmarks are detected
        try:
        ...
        # Display curl counter and stage
        # Setup status box
